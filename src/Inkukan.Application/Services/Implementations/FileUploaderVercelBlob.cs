...
public class FileUploaderVercelBlob(IFileChecker fileChecker, IBlobStorage blobStorage) : IFileUploader
{
    ...
    {
        EnsureFileIsSupportedType(fileName, content, supportedFileTypes);
    ...
    private void EnsureFileIsSupportedType(string fileName, byte[] content, params SupportedFileType[] supportedFileTypes)
    {
        if (fileChecker.FileIsSupportedType(fileName, content, supportedFileTypes)) return;
