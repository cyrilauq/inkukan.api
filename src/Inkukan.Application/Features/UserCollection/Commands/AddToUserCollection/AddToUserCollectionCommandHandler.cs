...
namespace Inkukan.Application.Features.UserCollection.Commands.AddToUserCollection
{
    ...
    { 
        public override async Task<bool> EnsureIsValidAsync(AddToUserCollectionCommand value)
        {
            bool baseResult = await base.EnsureIsValidAsync(value);

            if (await volumeRepository.GetByIdAsync(value.SerieVolumeId) is null)
                throw new EntityNotFoundException("No volume with the provided id were found");
            if (await userRepository.GetByIdAsync(value.UserId) is null)
